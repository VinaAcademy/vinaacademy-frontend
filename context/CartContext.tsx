...
interface CartContextType {
  ...
  error: Error | null
  addToCart: (
    cartItemRequest: { courseId: string; price: number },
...
export const CartProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  const enrichCartItem = async (
    item: CartItemDto,
  ): Promise<CartItemDisplay | null> => {
    try {
  ...
  const addToCart = async (
    ...
    try {
      ...
      const existingItem = cartItems.find(
        (item) => item.courseId === cartItemRequest.courseId,
      )
      if (existingItem) {
        if (!options?.silent) {
          toast({
      ...
      if (result) {
        ...
        const courseInfo = await getCourseWithInstructor(
          cartItemRequest.courseId,
        )
      ...
      setError(
        err instanceof Error ? err : new Error('Lỗi khi thêm vào giỏ hàng'),
      )
      if (!options?.silent) {
        toast({
          title: 'Không thể thêm vào giỏ hàng',
  ...
  const removeFromCart = async (itemId: number): Promise<boolean> => {
    ...
    try {
      ...
      setError(
        err instanceof Error ? err : new Error('Lỗi khi xóa khỏi giỏ hàng'),
      )
      toast({
        title: 'Không thể xóa khóa học',
        description: 'Đã xảy ra lỗi khi xóa khóa học khỏi giỏ hàng.',
  ...
  const updateCartItem = async (
    ...
    try {
      ...
      if (result) {
        ...
        setCartItems((prev) =>
          prev.map((item) =>
            item.id === itemId ? { ...item, price: result.price } : item,
          ),
        )
        toast({
      ...
      setError(
        err instanceof Error ? err : new Error('Lỗi khi cập nhật giỏ hàng'),
      )
      toast({
        title: 'Không thể cập nhật',
        description: 'Đã xảy ra lỗi khi cập nhật thông tin khóa học.',
  ...
  const applyCoupon = async (couponId: string): Promise<boolean> => {
    ...
    try {
      ...
      setError(
        err instanceof Error ? err : new Error('Lỗi khi áp dụng mã giảm giá'),
      )
      toast({
        title: 'Không thể áp dụng mã giảm giá',
        description: 'Đã xảy ra lỗi khi áp dụng mã giảm giá.',
