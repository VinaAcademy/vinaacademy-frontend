...
export default function ConversationsPage() {
  ...
  const { data: userSearchResults, isLoading: userSearchLoading } =
    useSearchUsers(debouncedSearchQuery, searchType === 'user')
  ...
  const handleStartChatWithUser = useCallback(
    async (userId: string, selectedUser: User) => {
      try {
        const conversation = await startDirectConversation(userId)
        if (conversation) {
          createSuccessToast(
            `Đã bắt đầu cuộc trò chuyện với ${selectedUser.fullName || selectedUser.username}`,
          )
          router.push(`/conversations/${conversation.id}`)
        } else {
