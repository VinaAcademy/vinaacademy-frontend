...
export default function FullPageLoadingOverlay({
    ...
    const isRouteIgnored = APP_CONFIG.LOADING_IGNORE_ROUTES.includes(pathname);
    if (isRouteIgnored) {
        return <>{children}</>;
    }
    ...
    return (
        <>
                ...
                >
                    <div className="flex flex-col items-center space-y-4 p-8 bg-white rounded-lg shadow-2xl">
                        ...
                        <div className="flex space-x-2">
                            ...
                            <div
                                className="w-2 h-2 bg-blue-500 rounded-full animate-bounce [animation-delay:0.15s]"></div>
                            <div
                                className="w-2 h-2 bg-blue-400 rounded-full animate-bounce [animation-delay:0.3s]"></div>
                        </div>
                    </div>
                </div>
            )}
            ...
            {error && (
                ...
                >
                    <div
                        className="flex flex-col items-center space-y-4 p-8 bg-white rounded-lg shadow-2xl max-w-md mx-4">
                        {/* Error Icon */}
