...
export const useCourseContent = (courseId?: string) => {
  ...
  const sectionsQueryKey: readonly ['sections', string] | [] = courseId
    ? SECTION_KEYS.byCourse(courseId)
    : []
  ...
  useMutation({
    mutationFn: async ({
      ...
      const updateData = {
        ...
        type: (lectureData.type?.toUpperCase() ||
          lecture.type.toUpperCase()) as LessonType,
        content:
          lectureData.content !== undefined
            ? lectureData.content
            : lecture.content || '',
        sectionId: sectionId,
        orderIndex:
        ...
        queryClient.setQueryData(
          ...
          (oldData: SectionDisplay[] | undefined) => {
            ...
            const sectionMap = new Map(
              oldData.map((section) => [section.id, section]),
            )
            ...
            ? oldData.map((s) => {
                if (s.id === sectionId) {
                  ...
                  const lectureMap = new Map(
                    s.lectures.map((lecture) => [lecture.id, lecture]),
                  )

                  // Create new array with updated order
                  const updatedLectures = lectureIds
                    .map((id, index) => {
