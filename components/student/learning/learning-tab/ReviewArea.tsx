...
const BeautifulSpinner = dynamic(() => import('@/components/ui/spinner'), {
  ssr: false,
...
const ReviewsArea: React.FC<ReviewsAreaProps> = ({
  ...
  const [editingReview, setEditingReview] = useState<CourseReviewDto | null>(
    null,
  )
  ...
  const [userOwnReview, setUserOwnReview] = useState<CourseReviewDto | null>(
    null,
  )
  ...
  useEffect(() => {
    ...
    ;(async () => {
      ...
      try {
        ...
        if (hasReviewed) {
          const userReview: CourseReviewDto | null =
            await getUserReviewForCourse(courseId)
          console.log('User review data:', userReview)

          if (userReview) {
            // Lưu review của user
            setUserOwnReview(userReview)
            ...
            setAllReviews((prev) => {
              ...
              if (exists) {
                return prev.map((r) =>
                  r.id === userReview.id ? userReview : r,
                )
              } else {
  ...
  useEffect(() => {
    const fetchPageData = async () => {
      ...
      try {
        ...
        const reviewsPage = await getCourseReviews(
          courseId,
          currentPage - 1,
          reviewsPerPage,
        )
  ...
  const handleDeleteReview = async (id: number) => {
    ...
    try {
      ...
      if (success) {
        ...
        // Nếu xóa review của chính mình, reset userOwnReview
        if (userOwnReview && userOwnReview.id === id) {
          setUserOwnReview(null)
        }
        ...
        if (newStats) {
          ...
          const percentages = counts.map((count) =>
            newStats.totalReviews > 0
              ? (count / newStats.totalReviews) * 100
              : 0,
          )
      ...
      <div className="flex flex-col md:flex-row gap-8">
        ...
        <div className="md:w-1/3">
          <div className="flex flex-col items-center">
            <div
              className="text-5xl font-bold text-[#b4690e]"
                  ...
                  <header className="flex items-center gap-4 mb-2">
                    ...
                    <div className="flex-1">
                      ...
                      >
                        ...
                        {review.userId === currentUserId ? '(Bạn)' : ''}
                      </div>
                      <div className="flex items-center gap-2">
                        <div
                          className="flex text-[#f69c08]"
                        ...
                        <time
                          ...
                          dateTime={String(
                            review.updatedDate || review.createdDate,
                          )}
                        >
                          {formatDate(
                            review.updatedDate || review.createdDate,
