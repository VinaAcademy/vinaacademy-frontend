...
const QuizContent: FC<QuizContentProps> = ({
  ...
  const [quizResult, setQuizResult] = useState<QuizSubmissionResultDto | null>(
    null,
  )
  const [quizSession, setQuizSession] = useState<QuizSession | null>(null)
  const [sessionExpired, setSessionExpired] = useState(false)
  const timerIntervalRef = useRef<NodeJS.Timeout | null>(null)
  const { setChatbotVisible } = useChatbotContext()

  // Hide chatbot when taking quiz, show when viewing results
  useEffect(() => {
    if (showResults) {
    ...
    return () => setChatbotVisible(true)
  }, [showResults, setChatbotVisible])
  ...
  useEffect(() => {
    const fetchQuizData = async () => {
      ...
      try {
        ...
        if (mappedQuiz.settings.randomizeQuestions) {
          mappedQuiz.questions = [...mappedQuiz.questions].sort(
            () => Math.random() - 0.5,
          )
        }
        ...
        if (session) {
            ...
            if (cachedAnswers && Object.keys(cachedAnswers).length > 0) {
              ...
              Object.values(cachedAnswers).forEach((answer) => {
                ...
                if (
                  ...
                  newSelectedAnswers[answer.questionId] =
                    answer.selectedAnswerIds
                }
              })
              ...
              console.log(
                ...
                Object.keys(cachedAnswers).length,
              )
            }
          } catch (err) {
          ...
          console.warn(
            'Failed to start quiz session. Using client-side timer instead.',
          )
        }
    ...
    const mapQuestionType = (
      type: string,
    ): 'single_choice' | 'multiple_choice' | 'text' | 'true_false' => {
      switch (type) {
        case 'SINGLE_CHOICE':
  ...
  const handleSelectOption = (questionId: string, optionId: string) => {
    ...
    if (quizSession && lectureId) {
      ...
      cacheQuizAnswer(lectureId, answerRequest).catch((error) =>
  ...
  const handleTextAnswer = (questionId: string, text: string) => {
    ...
    if (quizSession && lectureId) {
      ...
      cacheQuizAnswer(lectureId, answerRequest).catch((error) =>
        console.error('Failed to cache text answer:', error),
      )
    }
  }
  ...
  const parseQuizResults = () => {
    ...
    const results = quiz.questions.map((question) => {
      ...
      const apiAnswer = quizResult.answers.find(
    ...
    return {
      ...
      results,
      timeSpent:
        quizResult.startTime && quizResult.endTime
          ? new Date(quizResult.endTime).getTime() -
            new Date(quizResult.startTime).getTime()
          : 0,
    }
  }
            ...
            <div className="flex items-start space-x-4">
              ...
              <div className="flex-1">
                <h3 className="text-lg font-semibold text-gray-900">
                  Có câu hỏi bắt buộc chưa trả lời
                </h3>
                <p className="mt-2 text-gray-600">
                  Bạn chưa trả lời tất cả các câu hỏi bắt buộc. Bạn có chắc chắn
                  muốn nộp bài?
                </p>
