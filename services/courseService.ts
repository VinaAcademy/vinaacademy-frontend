...
import {
  CourseInstructorDto,
  CourseInstructorDtoRequest,
} from '@/types/instructor-course'

/** Helper to build Spring-style sort param */
const buildSort = (sortBy: string, sortDirection: 'asc' | 'desc') =>
...
export const getCourseWithInstructor = async (
  courseId: string,
): Promise<CourseWithInstructorDto | null> => {
  try {
    ...
    console.error(
      `Error fetching course with instructor for ID ${courseId}:`,
      error,
    )
    return null
  }
...
export const getCourseInstructors = async (
  ...
  try {
    ...
    console.error(
      `Error fetching instructors for course ID ${courseId}:`,
      error,
    )
    return null
  }
...
export const uploadImageAndCreateCourse = async (
...
export async function createInstructorCourse(
  course: CourseInstructorDtoRequest,
